...
function calendar($year = '', $month = '') {
	...
	//初期出力
	$html = <<<EOM
<table style = "width: 100%" id="calendar" border="3">
...
EOM;
	...
	for ($i = 1; $i < $l_day + 1;$i++) {
		...
		$query = sprintf('SELECT * FROM schedule WHERE start_time 
			BETWEEN "%s-%s-%s 00:00:00" AND "%s-%s-%s 23:59:59"', $year, $month, $i, $year, $month, $i);
    	...
    	//　各日のイベントおよび削除ボタンを表示する
		while($row = $result->fetch_assoc()) {
			$day_event .= "<li>";
			$day_event .= h($row['note']);
			$day_event .= "</li>";
			$day_event .= "<li style= \"display:inline;\" class = \"add\">";
			$day_event .= '<a class = "glyphicon glyphicon-trash" href="./del.php?schedule_id=';
			$day_event .= h($row['schedule_id']);
			$day_event .= '"></a>';
			$day_event .= "</li>";
		}
		$event_add_url = sprintf('./kut7days_add.php?day=%s-%s-%s', $year, $month, $i);
		...
		if ($i == date('j') && $year == date('Y') && $month == date('n')) {
			//現在の日付の場合
			$html .= "\t\t<td bgcolor = \"#D1E1FF\" id=\"today\">{$i}{$day_event}</td>\n";
		} else {
			//現在の日付ではない場合
			$html .= "\t\t<td bgcolor = \"#F7F7FF\">{$i}{$day_event}</td>\n";
		}
		//月末の場合
		if ($i == $l_day) {
...
// 背景画像変更処理
if (isset($_POST['bgChangeSubmit'])) {
	if (!empty($_POST['media'])) {
		// 変更処理
		// 変更するまえに既存のメディアIDかどうか確認
		$query = sprintf('SELECT COUNT(*) AS CNT FROM media WHERE media_id = "%s"', x($_POST["media"]));
		$result = $mysqli->query($query);
    	if (!$result) {
     		print('クエリが失敗しました。' . $mysqli->error);
      		$mysqli->close();
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" >
              ...
              <div class = "col-lg-12">
                ...
                <!-- Page Heading -->
                <div class="row">
                        <h1 class="page-header">KUT7Days設定</h1>
                    	<?php 
	...
	if(isset($_GET['year']) AND isset($_GET['month'])) {
		...
		echo "<ul class = \"stylemonth\"><li><a class = \"lastmonth btn btn-primary\" href=\"./kut7days.php?year=";
		echo date("Y", $last);
		echo "&month=";
		echo date("m", $last);
		echo "\">＜＜前月</a></li>";
		echo "<li><a class = \"nextmonth btn btn-primary\" href=\"./kut7days.php?year=";
		echo date("Y", $next);
		echo "&month=";
		echo date("m", $next);
		echo "\">翌月＞＞</a></li></ul>";
